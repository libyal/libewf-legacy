...
LIBRARY_TESTS="bit_stream checksum chunk_data chunk_table compression date_time date_time_values deflate empty_block error file_entry filename hash_sections hash_values huffman_tree io_handle media_values notify read_io_handle sector_range segment_table single_files write_io_handle";
...
run_test_with_input()
{
	...
	if ! test -d "input2";
	then
		echo "Test input2 directory not found.";

		return ${EXIT_IGNORE};
	fi
	local RESULT=`ls input2/* | tr ' ' '\n' | wc -l`;

	if test ${RESULT} -eq ${EXIT_SUCCESS};
	then
		echo "No files or directories found in the test input2 directory";

		return ${EXIT_IGNORE};
	fi

	local TEST_PROFILE_DIRECTORY=$(get_test_profile_directory "input2" "libewf-legacy");

	local IGNORE_LIST=$(read_ignore_list "${TEST_PROFILE_DIRECTORY}");

	RESULT=${EXIT_SUCCESS};

	for TEST_SET_INPUT_DIRECTORY in input2/*;
	do
